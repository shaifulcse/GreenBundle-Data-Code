...
public class Angulo extends Activity implements SensorEventListener
{
    ...
    switch (evt.sensor.getType ())
      {
        case Sensor.TYPE_ACCELEROMETER:
          ...
          bundler.refGrav.add(state.refGrav);
          bundler.lastGrav.add(state.lastGrav);
          bundler.gravityDegree.add(gravityDegree);
          bundler.gravityPercent.add(gravityPercent);

          break;

        case Sensor.TYPE_MAGNETIC_FIELD:
          state.lastMagn = new Vector (evt.values);
          bundler.refMagn.add(state.refMagn);
          bundler.lastMagn.add(state.lastMagn);
          bundler.magneticDegree.add(magneticDegree);
          bundler.magneticPercent.add(magneticPercent);
          bundler.gravityPercent.add(gravityPercent);
  ...
  public void bunldingDraw(){
    ...
    clearSavedData();
    int size=bundler.tempRefGrav.size();

    for(int i=0;i<size;i++){

      setDisplays (bundler.tempRefGrav.get(i), bundler.tempLastGrav.get(i),
                   bundler.tempGravityDegree.get(i), bundler.tempGravityPercent.get(i));
      updateCombined ();

    }


    size=bundler.tempRefMagn.size();

    for(int i=0;i<size;i++){

      setDisplays (bundler.tempRefMagn.get(i), bundler.tempLastMagn.get(i),
              bundler.tempMagneticDegree.get(i), bundler.tempMagneticPercent.get(i));
      updateCombined ();
  ...
  public synchronized void clearSavedData(){

    bundler.copySavedData();
    bundler.initialize();
