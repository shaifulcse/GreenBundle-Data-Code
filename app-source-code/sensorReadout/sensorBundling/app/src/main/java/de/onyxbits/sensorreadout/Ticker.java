...
class Ticker extends Thread implements SensorEventListener {
  ...
  private static ArrayList<MySensorEvent> collectedEvents= new ArrayList<MySensorEvent>();
  /** 
  ...
  static int waitTime=500; //in ms
  static int countMax=waitTime/SLEEPTIME;
    /**
  ...
  public void run() {
    ...
    if (worker!=null) {
      ...
      if (currentEvent!=null ) {
          ...
          MySensorEvent myeventsensor= new MySensorEvent(currentEvent.sensor,
                  currentEvent.values, currentEvent.accuracy, currentEvent.timestamp);

          collectedEvents.add(myeventsensor);
          if (counter>=countMax) {
              int size=collectedEvents.size();

              for(int i=0;i<size;i++){

                  activity.onTick(collectedEvents.get(i));
              }
